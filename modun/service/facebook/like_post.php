...
if ($_REQUEST) {
    ...
    $type   = $_REQUEST['type'];
    if ($type === 'buff-like-post-facebook') {
        ...
        if ($user_point < $buff_payment) {
            ...
            $return['error'] = 1;
            $return['msg']   = 'The quantity field must be at least ' . $check_service['service_min_amount'] . 'Like.';
            die(json_encode($return));
        } else if ($buff_amount > $check_service['service_max_amount']) {
            $return['error'] = 1;
            $return['msg']   = 'The maximum number field is ' . $check_service['service_max_amount'] . 'Like.';
            die(json_encode($return));
        }else {
